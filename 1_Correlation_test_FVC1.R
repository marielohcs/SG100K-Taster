...
t.test(DSM7_FVC_1 ~ FREG7_GENDER2_LABEL)
ttest_gender <- t.test(DSM7_FVC_1 ~ FREG7_GENDER2_LABEL)

correlation_age <- cor.test(bcos_data$FREG8_AGE, bcos_data$DSM7_FVC_1, method = c("pearson"))$estimate


pvalue_gender <- dt(ttest_gender$statistic,df=(ttest_gender$parameter))
...
C <- ggplot(bcos_data, mapping = aes(x = FREG8_AGE2 , y = DSM7_FVC_1)) + 
  geom_point() +
  labs(x = "FREG8_AGE", y = "DSM7_FVC_1", title = "FREG8_AGE & DSM7_FVC_1") +
  geom_smooth(method='lm', formula = y~x, color = "black") + 
  annotate("text", x =70, y = 6, label = paste("R=",(round(correlation_age,3))))

D <- ggplot(bcos_data, mapping = aes(x = as.factor(FREG7_GENDER2_LABEL) , y = DSM7_FVC_1)) + 
  geom_boxplot() +
