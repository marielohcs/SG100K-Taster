...
t.test(DSM8_FEV1_1 ~ FREG7_GENDER_LABEL)
ttest_gender <- t.test(DSM8_FEV1_1 ~ FREG7_GENDER_LABEL)

correlation_age <- cor.test(bcos_data$FREG8_AGE, bcos_data$DSM8_FEV1_1, method = c("pearson"))$estimate
...
C <- ggplot(bcos_data, mapping = aes(x = FREG8_AGE , y = DSM8_FEV1_1)) + 
  geom_point() +
  labs(x = "FREG8_AGE", y = "DSM8_FEV1_1", title = "FREG8_AGE & DSM8_FEV1_1") +
  geom_smooth(method='lm', formula = y~x, color = "black") + 
  annotate("text", x =70, y = 5, label = paste("R=",(round(correlation_age,3))))

D <- ggplot(bcos_data, mapping = aes(x = as.factor(FREG7_GENDER_LABEL) , y = DSM8_FEV1_1)) + 
  geom_boxplot() +
